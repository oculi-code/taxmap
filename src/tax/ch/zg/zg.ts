...
import data from './zg.data.json'
import municipalities from './zg.municipalities.json'

/**
 * Zug: 3 layers, all percent-of-"einfache Steuer" — cantonal, municipal,
 * church. Reformed church tax is actually a uniform canton-wide 6% (one
 * body, unlike Catholic's per-municipality parishes), but it's already the
 * same value on every municipality row, so it's just looked up like any
 * other faith — no special-casing needed here. A narrow 4th layer
 * (Bürgergemeinden, taxing only citizens with local Bürgerrecht) exists but
 * isn't modeled — this app has no input for that status.
 */
export const zg: CantonTaxModule = {
  computeComponents(input, taxableIncome, bfsNumber): TaxComponent[] {
    ...
    const components: TaxComponent[] = [
      { label: cantonalTaxLabel('ZG'), baseTax, multiplier: cantonalFraction, amount: baseTax * cantonalFraction },
    ]
