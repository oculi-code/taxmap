...
import data from './ag.data.json'
import municipalities from './ag.municipalities.json'

/**
 * Aargau: 3 layers, all percent-of-"einfache Steuer" — cantonal (§43/§55
 * StG), municipal, church. One shared wealth-tax schedule (no single/married
 * split, unlike ZH). Some municipalities also carry a christ-catholic rate,
 * preserved in the data even though the app's faith options don't yet cover it.
 */
export const ag: CantonTaxModule = {
  computeComponents(input, taxableIncome, bfsNumber): TaxComponent[] {
    ...
    const components: TaxComponent[] = [
      { label: 'Cantonal tax (AG)', baseTax, multiplier: cantonalFraction, amount: baseTax * cantonalFraction },
    ]
